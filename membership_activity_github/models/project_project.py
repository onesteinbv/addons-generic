...
def use_github(method):
    ...
    def _wrap(self, *args, **kwargs):
        ...
        if not self.github_full_name:
            raise UserError(self.env._("Github Fullname is not set."))
...
class Project(models.Model):
    ...
    @api.model
    def _github_datetime_to_odoo(self, datetime):
                ...
                vals_list.append(
                    {
                        ...
                        "url": commit.html_url,
                        "date": self._github_datetime_to_odoo(
    ...
    def get_github_issues(self):
        ...
        for project in self:
            ...
            count = 0
            for _ in repo.get_issues(**params):
                count += 1
            # issues = repo.get_issues(**params)
            # count = issues.totalCount # To be used/uncommented when the issue
            # at https://github.com/PyGithub/PyGithub/issues/3257 is resolved
    ...
    def get_github_issues_immediately(self, start_page, end_page, since):
        self.ensure_one()
        hub = self._create_github_instance()
        params = {"sort": "created", "direction": "asc", "state": "all", "since": since}
                    ...
                    vals_list.append(
                        {
                            ...
                            "type_id": type_id,
                            "date": self._github_datetime_to_odoo(issue.created_at),
                            "url": issue.html_url,
                        }
                    )
                ...
                vals_list.append(
                    {
                        ...
                        "type_id": activity_type.id,
                        "date": self._github_datetime_to_odoo(comment.created_at),
