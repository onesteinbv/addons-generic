...
export class MassMailingTrip extends Trip {
    setup() {
        ...
        this.addStep({
            selector:
                ".o_cell:has(input#subject_0), .o_cell:has(label[for='subject_0'])",
        ...
        this.addStep({
            ...
            beforeHighlight: async () => {
                ...
                $(
                    "[data-menu-xmlid='mass_mailing.mass_mailing_mailing_list_menu']"
                ).click();
                await waitUntilAvailable(
                    "[data-menu-xmlid='mass_mailing.menu_email_mass_mailing_lists']"
                );
        ...
        this.addStep({
            selector: ".o_list_button_add, .o-kanban-button-new",
            content: _t("Click on the Create button."),
            beforeHighlight: async () => {
                await doAction("mass_mailing.action_view_mass_mailing_lists", {
        ...
        this.addStep({
            selector:
                ".o_cell:has(input#is_public_0), .o_cell:has(label[for='is_public_0'])",
            content: _t(
        ...
        this.addStep({
            selector: ".o_list_button_add, .o-kanban-button-new",
            content: _t("Lets add a Recipient."),
            beforeHighlight: async () => {
        ...
        this.addStep({
            selector:
                ".o_cell:has(input#partner_id_0), .o_cell:has(label[for='partner_id_0'])",
            content: _t(
            ...
            beforeHighlight: async () => {
                $(".o_list_button_add, .o-kanban-button-new").click();
                await waitUntilAvailable(
                    ".o_form_view_container .o_cell:has(input#partner_id_0)"
                );
            },
        });
        ...
        this.addStep({
            ...
            beforeHighlight: async () => {
                await doAction("mass_mailing.mailing_mailing_action_mail", {
                    viewType: "form",
                });
        ...
        if (odoo.debug) {
            this.addStep({
                ...
                content: _t("Use A/B testing to optimize your mailing"),
            });
        }

        this.addStep({
            selector: "button[name='action_test']",
            content: _t("You can send a test mail to yourself by clicking 'Test'."),
        });
