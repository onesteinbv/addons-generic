...
class StockPicking(models.Model):
    ...
    sendcloud_parcel_status = fields.Selection(
        [("passed", "Passed"), ("failed", "Failed")], default=None
    )
    ...
    def _log_sendcloud_parcel_errors(self, err_msg):
        self.write({"sendcloud_parcel_status": "failed"})
        ...
        if is_outside_eu or state_requires_hs_code:
            ...
            if not parcel_item_outside_eu.get("hs_code"):
                ...
                if self._context.get("raise_error"):
                    ...
                    move.picking_id._log_sendcloud_parcel_errors(err_msg)
                    return {}
    ...
    def _sendcloud_check_collo_weight(self, weight):
        ...
        if min_weight and max_weight and not (min_weight <= weight <= max_weight):
            ...
            if self._context.get("raise_error"):
                ...
                self._log_sendcloud_parcel_errors(err_msg)
                return "Error"
        return weight
    ...
    def button_create_sendcloud_labels(self):
        ...
        if (
            ...
            self.write({"sendcloud_parcel_status": "passed"})
    ...
    def _sendcloud_sync_multiple_parcels(self, integration, parcel_vals_list):
        ...
        try:
            ...
            self._log_sendcloud_parcel_errors(e)
            return {}
        if response.get("error"):
            self._log_sendcloud_parcel_errors(
                _("Sendcloud: %s") % response.get("error").get("message")
            )
        if response.get("failed_parcels"):
            for failed in response.get("failed_parcels"):
                self._log_sendcloud_parcel_errors(
                    _("Sendcloud- %(parcel)s:\n%(errors)s\n\n")
                    % (
                        {
    ...
    def _sync_picking_to_sendcloud(self):
        ...
        for picking in pickings:
            integration = picking.carrier_id.sendcloud_integration_id
            shipment_vals_list = picking.with_context(
                raise_errors=True
            )._prepare_sendcloud_parcels_from_picking()
