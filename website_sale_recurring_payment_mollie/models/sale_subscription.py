...
class SaleSubscription(models.Model):
    ...
    def update_sale_subscription_payments_and_subscription_status(self, date_ref):
        ...
        if self.payment_provider_subscription_id.provider_id.code != 'mollie':
            return super().update_sale_subscription_payments_and_subscription_status(date_ref)
        ...
        if mollie_customer_id:
            ...
            subscription = customer.subscriptions.get(self.payment_provider_subscription_id.reference)
            if subscription:
                if subscription.get('STATUS_CANCELED', False) and subscription.get('STATUS_CANCELED',
                                                                                   '') == subscription.get('status',
                                                                                                           ''):
                    # As the subscription is cancelled, end the subscription
                    if 'canceledAt' in subscription.keys():
                        canceled_date = datetime.strptime(subscription.get('canceledAt')[0:19],
                                                          "%Y-%m-%dT%H:%M:%S").date()
                    else:
                        canceled_date = date_ref
                    vals = {"date": canceled_date, "recurring_next_date": False,
                    ...
                    self.write(vals)
                    msg = _(
                        "The mollie subscription for this subscription has been terminated on %s" % (canceled_date))
                ...
                if subscription_payments and subscription_payments.get('_embedded'):
                    payment_list = subscription_payments['_embedded'].get('payments', [])
                    for payment in payment_list:
                        payment_transaction_obj._process_payment_provider_subscription_recurring_payment(self, payment)
        return True
    ...
    def terminate_payment_provider_subscription(self):
        ...
        subscription = customer.subscriptions.delete(self.payment_provider_subscription_id.reference)
        if subscription:
            canceled_date = False
            if 'canceledAt' in subscription.keys():
                canceled_date = datetime.strptime(subscription.get('canceledAt')[0:19], "%Y-%m-%dT%H:%M:%S")
            msg = _("The mollie subscription for this subscription has been terminated on %s" % (canceled_date))
            self.sudo().message_post(body=msg)
