...
class PaymentTransaction(models.Model):
        ...
        if self.sale_order_ids:
            ...
            if partner_obj.mollie_customer_id:
                ...
                customer_id_data = self.provider_id.with_context(
                    partner=partner_obj.id)._api_mollie_create_customer_id()
    ...
    def _create_mollie_order_or_payment(self):
        ...
        if ('sale_order_ids' in self._fields and self.sale_order_ids and self.sale_order_ids.group_subscription_lines()
                and method_record.supports_payment_api and method_record.supports_order_api):
    ...
    def _create_subscription_for_payment_provider(self,subscription, payment_data):
        ...
        interval = '%s %s' % (subscription_template.recurring_interval, subscription_template.recurring_rule_type)
        ...
        try:
            subscription = customer.subscriptions.create(data)    # Uncomment this when ready (issue is that localhost doesn't work with Mollie)
    ...
    def _get_payment_transaction(self, subscription, payment):
        ...
        if subscription.payment_provider_subscription_id.provider_id.code == 'mollie':
            payment_transaction = self._get_payment_transaction_by_provider_reference(payment['id'],
            ...
            if not payment_transaction:
                ...
                currency = self.env["res.currency"].search(
                    [("name", "=", payment.get('amount', {}).get("currency", ""))], limit=1) if payment.get('amount',
                                                                                                            {}).get(
                    "currency", "") else subscription.currency_id
                payment_transaction = self.create(
                    self._prepare_vals_for_recurring_payment_transaction_for_subscription(provider_reference, amount,
