...
class AccountGroup(models.Model):
    ...
    allowed_journal_ids = fields.Many2many(
        comodel_name="account.journal", string="Saved Allowed Journals",
    ...
    active_allowed_journal_ids = fields.Many2many(
        ...
        compute="compute_active_allowed_journals",
        inverse="set_allowed_journals"
    )
    ...
    def accounts_set_allowed_journals(self):
        for rec in self:
            if rec.auto_allowed_journals:
                rec.with_context(group_allowed_journal_change=True).account_ids.allowed_journal_ids = rec.active_allowed_journal_ids

    @api.depends('parent_id','parent_id.allowed_journal_ids')
    @api.onchange('parent_id', 'allowed_journal_ids')
    def compute_active_allowed_journals(self):
        for rec in self:
            rec.active_allowed_journal_ids = rec.get_all_allowed_journal_ids()

    def set_allowed_journals(self):
        for rec in self:
