...
options.registry.WebsiteMembershipRegistrationPage = options.Class.extend({
     ...
     */
    start: function () {
        this.max_cv_file_size = parseInt(
            this.$target.closest("[data-max-cv-file-size]").data("max-cv-file-size"),
            10
        );
        this.cv_file_formats = this.$target
            .closest("[data-cv-file-formats]")
            .data("cv-file-formats");
        this.membership_group_style = this.$target
            .closest("[data-membership_group-style]")
            .data("membership_group-style");
